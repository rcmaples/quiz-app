...
function startGame() {
  ...
  $('#js-quiz-start').submit(function(event) {
    ...
    // hide the start page
    $('.js-start-unhidden')
      .removeClass('js-start-unhidden')
      .addClass('js-start-hidden');
    //unhide the questions page
    $('.js-questions-hidden')
      .removeClass('js-questions-hidden')
      .addClass('js-questions-unhidden');
  });
}
...
function randomizeAnswers() {
  console.log(
    `The lenght of the incorrectAnswerKeyis: ${incorrectAnswerKey.length}`
  );
...
function questionImageLoader(qnum) {
  if (qnum === 0) {
    $('.questionIcon').html(
      `<img alt="A line drawing of Jupiter" src="./images/jupiter.svg">`
    );
  } else if (qnum === 1) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a 3 headed hydra" src="./images/hydra.svg">`
    );
  } else if (qnum === 2) {
    $('.questionIcon').html(
      `<img alt="A line drawing of Juno." src="./images/juno.svg">`
    );
  } else if (qnum === 3) {
    $('.questionIcon').html(
      `<img alt="Sillhouette of some muses" src="./images/muses.svg">`
    );
  } else if (qnum === 4) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a mermaid" src="./images/mermaid.svg">`
    );
  } else if (qnum === 5) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a bull's head" src="./images/taurus.svg">`
    );
  } else if (qnum === 6) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a trojan horse" src="./images/trojanhorse.svg">`
    );
  } else if (qnum === 7) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a centaur" src="./images/centaur.svg">`
    );
  } else if (qnum === 8) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a minotaur" src="./images/minotaur.svg">`
    );
  } else if (qnum === 9) {
    $('.questionIcon').html(
      `<img alt="A line drawing of a pegasus" src="./images/pegasus.svg">`
    );
  }
}
...
$('.modal').on('click', '[rel="modal:close"]', function(event) {
  ...
  if (
    $(this)
      .parent()
      .hasClass('js-modal-lastQuestion')
  ) {
    return;
  } else {
    //if (
...
$(document).on('click', '.js-correct-answer', function(event) {
  ...
  if ($(this)[0].hasAttribute('lastQuestion')) {
    $('.js-correct-answer-modal')
      ...
      .html(
        `<p>Good Job!</p><a href="#close-modal" tabIndex="0" rel="modal:close">End Game</a>`
      )
      .modal({ escapeClose: false, clickClose: false });
    $('.js-questions-unhidden')
      .removeClass('js-questions-unhidden')
      .addClass('js-questions-hidden');
  } else {
    $('.js-correct-answer-modal')
      .html(
        `<p>Good Job!</p><a href="#close-modal" tabIndex="0" rel="modal:close">Next Question</a>`
      )
      .modal({ escapeClose: false, clickClose: false });
    $('.js-questions-unhidden')
      .removeClass('js-questions-unhidden')
...
$(document).on('click', '.js-modal-lastQuestion', function(event) {
  ...
  $('.js-questions-unhidden')
    ...
    .addClass('js-questions-hidden');
  $('.js-results-hidden')
    .removeClass('js-results-hidden')
    .addClass('js-results-unhidden');
  $('.js-correct-answer-modal')
    .removeClass('js-modal-lastQuestion')
    .addClass('js-modal-notTheLastQuestion');
  $('.js-incorrect-answer-modal')
    .removeClass('js-modal-lastQuestion')
    .addClass('js-modal-notTheLastQuestion');
...
function displayResults(score) {
  if (score < 60) {
    $('.js-results-tex').html(
      "Better luck next time.</p><p>Why don't you play again?"
    );
    $('.js-results-score').html(
      `You're final score is ${numberCorrect} / 100!`
    );
  } else if (score < 80) {
    //do other stuff
...
function resetGame() {
  ...
  $('#js-quiz-answers')
    .find('button')
    .removeAttr('lastQuestion');
  $('.js-results-unhidden')
    .removeClass('js-results-unhidden')
    .addClass('js-results-hidden');
  $('.js-start-hidden')
    .removeClass('js-start-hidden')
    .addClass('js-start-unhidden');
  $('.js-incorrect-answer-modal')
    .removeClass('js-modal-lastQuestion')
    .addClass('js-modal-notTheLastQuestion');
  $('.js-correct-answer-modal')
